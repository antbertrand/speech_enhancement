...
def main():
    ...
    noiseDataset = NoiseDataset(csv_noise_train, fs=fs)

    in_raw_tf = noiseDataset.add_noise_snr
    in_raw_tf_kwargs = {"fs": fs, "snr": snr}
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        raw_target, fs = read_wav(self.raw_paths[index])
...
class NoiseDataset(Dataset):
    ...
    def __init__(self, csv_noise, fs=None):
        ...
        # TODO see if allows to fix behavior
        torch.random.manual_seed(int(1e9*time.time()))

        self.resampler = torchaudio.transforms.Resample(
            orig_freq=16000,   # dummy values
            new_freq=self.fs if self.fs is not None else 16000)

        self.sounds_len_in_s = self.__get_sounds_len_in_s()

    def __get_sounds_len_in_s(self):
        ...
        lengths = np.dot(1/rates, n_samples)
    ...
    def __getitem__(self, index):
        ...
        Arguments:
            index {int} -- File index, according to the given csv file.
    ...
    def gen_noise(self, noise_len, fs=None):
        ...
        # --- generate random index from len in s
        # Check valid sounds index wrt len in s
        noise_len_in_s = noise_len * (1/self.fs)
        valid_idxs = np.array([self.sounds_len_in_s > noise_len_in_s]) # TODO not sure working for multiple noise files dataset

        if not any(valid_idxs):
            print('WARNING : Aucun bruit de taille suffisante dans la database')

        nb_samples = self.sounds_len_in_s[valid_idxs] * self.fs
        nb_samples_cumsum = nb_samples.cumsum()
        idx = np.random.randint(0, nb_samples_cumsum[-1])
        snd_idx = np.searchsorted(nb_samples_cumsum, idx)
        
        if nb_samples[snd_idx] > noise_len:
            pass # TODO si pas la place à droite alors eborder à gauche (y'aura la place car on a choper les indices valides)
